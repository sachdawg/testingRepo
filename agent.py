...
##            force = self.pursuit(self.world.hunter)
    ...
    def flee(self, hunter_pos):
        ...
        panic_distance = 100  # Adjust the panic distance as needed

        # Calculate the distance between the agent and the hunter
        distance = self.pos.distance(hunter_pos)

        # If the distance is less than the panic distance, flee
        if distance < panic_distance:
            desired_vel = (self.pos - hunter_pos).normalise() * self.max_speed
            return (desired_vel - self.vel)
        
        # If the distance is greater than or equal to the panic distance, no need to flee
        return Vector2D()
